...
# Laboratorio de la Dra. Mariela Maldonado

## Av. Independencia 3650
...
#### **Mar del Plata, 6 de enero de 2022**
...
Base <- read_xlsx("BASE MALDONADO 2018 2019 2020 y parte de 2021.xlsx") 
...
POSITIVOS <- Base %>% filter(AÑO %in% c("2018", "2019", "2020"),
  ...
  select(Paciente, SEXO, `FECHA RECEP`, EDAD,TOPOGRAFIA, DIAGNOSTICO, AÑO)
...
TOPOGRAFIAS_sin_multiples <- POSITIVOS %>%  
  mutate(SITIO_DE_PRIMARIO = case_when(
    ...
    str_detect(string = TOPOGRAFIA, pattern = "ESTOMAGO") & 
      ...
      str_detect(string = DIAGNOSTICO, pattern = "PANCREA", negate = T) ~ "ESTOMAGO",
...
graf1 <- TOPOGRAFIAS_sin_multiples %>% 
  ...
  scale_fill_manual(values = c("Black", "turquoise2", "tomato2","springgreen2",
                               "pink2", "goldenrod","chocolate3", "chartreuse",
                               "orange","olivedrab")) +
...
graf2 <- TOPOGRAFIAS_sin_multiples %>% 
  ...
  ggplot(aes(x = fct_rev(fct_reorder(SITIO_DE_PRIMARIO, n, min)), y = n,fill = SITIO_DE_PRIMARIO)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = c("turquoise2", "tomato2", "yellow","springgreen2",
                               "pink2","goldenrod","chocolate3","violetred3",
...
graf3 <- TOPOGRAFIAS_sin_multiples %>% 
  ...
  ggplot(aes(x = fct_rev(fct_reorder(SITIO_DE_PRIMARIO, n, min)), y = n,fill = SITIO_DE_PRIMARIO)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = c("red", "turquoise2", "tomato2", "yellow","springgreen2","pink2","goldenrod","chocolate3","chartreuse", "olivedrab")) +
  theme(axis.text.x = element_text(angle = 45, hjust=1))	+ 
  scale_y_continuous(limits = c(0,200), breaks = seq(0,200, by = 20)) +
  xlab("") +
  ylab("Frecuencia") +
