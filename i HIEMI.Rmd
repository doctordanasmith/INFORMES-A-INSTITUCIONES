...
Castelli 2450 -- Mar del Plata -- Buenos Aires
...
Base <- read_csv("2022 - 02 - 10.csv")

Base_HIEMI  <- Base %>% filter(`Institución 1 (desc)` == "HIEMI") %>%
...
TOPOGRAFIAS_sin_multiples <- Base_HIEMI %>% filter(AÑO %in% c("2018", "2019", "2020"))
...
```{r Comp, echo=FALSE, fig.align="center", fig.width=9, message=FALSE, warning=FALSE, dpi=600}
...
graf1 <- TOPOGRAFIAS_sin_multiples %>% 
  ...
  scale_fill_manual(values = c( "tomato2","steelblue4","olivedrab","pink2","gray40","chocolate3", "springgreen2","turquoise2", "goldenrod","salmon2")) +
...
graf2 <- TOPOGRAFIAS_sin_multiples %>% 
  ...
  ggplot(aes(x = fct_rev(fct_reorder(SITIO_DE_PRIMARIO, n, min)), y = n,fill = SITIO_DE_PRIMARIO)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = c("blue", "tomato2","steelblue4","pink2","turquoise2","chartreuse","salmon2", "violetred3")) +
  theme(axis.text.x = element_text(angle = 45, hjust=1))	+
  scale_y_continuous(limits = c(0,15), breaks = seq(0,15, by = 5)) +
  xlab("") +
  ylab("Frecuencia") +
...
graf3 <- TOPOGRAFIAS_sin_multiples %>% 
  ...
  ggplot(aes(x = fct_rev(fct_reorder(SITIO_DE_PRIMARIO, n, min)), y = n,fill = SITIO_DE_PRIMARIO)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = c("tomato2","pink2","black", "turquoise2")) +
  theme(axis.text.x = element_text(angle = 45, hjust=1))	+ 
  scale_y_continuous(limits = c(0,15), breaks = seq(0,15, by = 5)) +
  xlab("") +
  ylab("Frecuencia") +
...
TOPOGRAFIAS_sin_multiples <- Base_HIEMI %>% filter(AÑO %in% c("2018", "2019", "2020")) %>%  rename(SEXO = `Sexo (desc)`, EDAD = Edad)
