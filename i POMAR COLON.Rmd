...
# Laboratorio de Clínica y Maternidad Colón
...
Base <- read_csv("BASE POMAR COLON 2018-2019-2020.csv",
                 locale = locale(encoding = "WINDOWS-1252")) 


Base <- Base %>% rename(`No Documento` = numero_doc, DIAGNOSTICO = diagnostico ) %>%  
  mutate(`NRO DOC`= as.character(`No Documento`)) %>% 
  mutate(fecha_nac= dmy_hm(fecha_nac), fecha_recepcion = dmy_hm(fecha_recepcion )) %>% 
  mutate(fecha_nac =  as_date(fecha_nac),
         `FECHA RECEP` = as_date(fecha_recepcion)
  , CANCER = case_when(str_detect(string = DIAGNOSTICO, pattern = "CARCINOMA") ~ "SI",
...
POSITIVOS <- Base %>% filter(AÑO %in% c("2018", "2019", "2020")) %>% 
  filter(CANCER == "SI")%>% distinct(`NRO DOC`, .keep_all = T) 
...
```{r, Distribucion, echo=FALSE, message=FALSE, warning=FALSE}
...
Base <- Base %>% rename(`No Documento` = numero_doc, DIAGNOSTICO = diagnostico, SEXO = sexo, EDAD = edad, TOPOGRAFIA = topografias ) %>%  
  mutate(`NRO DOC`= as.character(`No Documento`)) %>% 
  mutate(fecha_nac= dmy_hm(fecha_nac), fecha_recepcion = dmy_hm(fecha_recepcion )) %>% 
  mutate(`FECHA NAC` =  as_date(fecha_nac),
         `FECHA RECEP` = as_date(fecha_recepcion), 
         TOPOGRAFIA = str_to_upper(TOPOGRAFIA), DIAGNOSTICO = str_to_upper(DIAGNOSTICO),CANCER = case_when(str_detect(string = DIAGNOSTICO, pattern = "CARCINOMA") ~ "SI",
                                                            str_detect(string = DIAGNOSTICO, pattern = "LINFOMA") ~ "SI",
                                                            str_detect(string = DIAGNOSTICO, pattern = "CARCINOIDE") ~ "SI",
                                                            str_detect(string = DIAGNOSTICO, pattern = "MELANOMA") ~ "SI",
                                                            str_detect(string = DIAGNOSTICO, pattern = "LEUCEMIA") ~ "SI",
...
POSITIVOS <- Base %>% filter(CANCER == "SI") %>% 
  select(SEXO, `NRO DOC`, `FECHA NAC`,`FECHA RECEP`, EDAD,TOPOGRAFIA, DIAGNOSTICO, AÑO) 
...
TOPOGRAFIAS_sin_multiples <- POSITIVOS %>%  
  mutate(SITIO_DE_PRIMARIO = case_when(
    ...
    str_detect(string = TOPOGRAFIA, pattern = "RIÑ| REN|RI¥ON") &
    ...
    str_detect(string = TOPOGRAFIA, pattern = "ESOFAGO") |
      str_detect(string = DIAGNOSTICO, pattern = "BARRET") ~ "ESOFAGO", 
    ...
    str_detect(string = DIAGNOSTICO, pattern = "ADENOCARCINOMA ENDOMETROIDE") ~ "ADENOCA. UTERINO",
...
graf1 <- TOPOGRAFIAS_sin_multiples %>% 
  ...
  scale_fill_manual(values = c("turquoise2", "tomato2", "springgreen2","pink2", "blue", "goldenrod","chocolate3","violetred3","chartreuse", "olivedrab")) +
...
graf2 <- TOPOGRAFIAS_sin_multiples %>% 
  ...
  ggplot(aes(x = fct_rev(fct_reorder(SITIO_DE_PRIMARIO, n, min)), y = n,fill = SITIO_DE_PRIMARIO)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = c("turquoise2", "tomato2", "springgreen2","pink2","goldenrod","chocolate3", "violetred3","chartreuse", "orange", "olivedrab")) +
  theme(axis.text.x = element_text(angle = 45, hjust=1))	+
  scale_y_continuous(limits = c(0,300), breaks = seq(0,300, by = 50)) +
  xlab("") +
  ylab("Frecuencia") +
...
Base <- Base %>% rename(`No Documento` = numero_doc, DIAGNOSTICO = diagnostico, SEXO = sexo, EDAD = edad, TOPOGRAFIA = topografias ) %>%  
  ...
  mutate(`FECHA NAC` =  as_date(fecha_nac),
         `FECHA RECEP` = as_date(fecha_recepcion), 
  TOPOGRAFIA = str_to_upper(TOPOGRAFIA), DIAGNOSTICO = str_to_upper(DIAGNOSTICO),
CANCER = case_when(str_detect(string = DIAGNOSTICO, pattern = "CARCINOMA") ~ "SI",
                                                            str_detect(string = DIAGNOSTICO, pattern = "LINFOMA") ~ "SI",
                                                            str_detect(string = DIAGNOSTICO, pattern = "CARCINOIDE") ~ "SI",
                                                            str_detect(string = DIAGNOSTICO, pattern = "MELANOMA") ~ "SI",
